...
class AemSmcCrit(AemIsJointCrit):
        ...
        for i in range(1, self.dim):
            ...
            for j in range(batch_size):
                ...
                if ess < (num_samples / 2):
                    with torch.no_grad():
                        ancestor_inds = Categorical(probs=torch.exp(log_w_y_s)).sample(sample_shape=torch.Size((num_samples,)))
            ...
            for j in range(batch_size):
                for k in range(num_samples):
                    ...
                    log_q_y_s[j, k], _, y_s[k, :] = self._proposal_log_probs(y_s[k, :].reshape(1, -1), i, num_observed=0)
    ...
    def log_part_fn(self, return_ess=False, return_all=False):
        ...
        log_normalizer, _, _, ess = self.inner_smc(1, self.num_neg_samples_validation, None) # temporary_smc
