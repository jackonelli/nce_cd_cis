...
from src.data.data_uci.uciutils import preprocess_and_save_hepmass, get_data_root
...
class HEPMASSDataset(Dataset):
    def __init__(self, split='train', frac=None, num_dims=None, scaling='standardize'):
        ...
        if scaling == 'standardize':
            ...
            path = os.path.join(get_data_root(), 'data_norm', 'hepmass/{}.npy'.format(split))
        ...
        try:
            ...
            preprocess_and_save_hepmass(scaling)
            print('Done!')
            self.data = np.load(path).astype(np.float32)
            print("Data loaded from:" + str(path))
