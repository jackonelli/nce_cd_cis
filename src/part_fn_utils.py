...
def cond_x2_log_cond_unnorm_weights(
        y: Tensor, x: Tensor, yp: Tensor, log_unnorm_distr, log_noise_distr
) -> Tensor:
    """Compute log weights log w(y|yp, x) = log_p_tilde_theta(y|x) / log_p_n(y| yp, x) for cond. model and noise distr.
    ...
    return (log_unnorm_distr((y, x)) - log_noise_distr(y, (yp, x)))


def cond_x2_log_cond_unnorm_weights_ratio(
        y: Tensor, x: Tensor, yp: Tensor, log_unnorm_distr, log_noise_distr
) -> Tensor:
    """Compute log weights ratio log w(y|yp, x)/w(yp|y, x) with w(y|yp, x) = p_tilde_theta(y | x) / p_n(y| yp, x) for
    cond. model and noise distr.
    ...
    """
    return (
            cond_x2_log_cond_unnorm_weights(y, x, yp, log_unnorm_distr, log_noise_distr)
            - cond_x2_log_cond_unnorm_weights(yp, x, y, log_unnorm_distr, log_noise_distr)
    )
