...
def main(args):
    ...
    crits = ['CIS', 'CSMC']
    crit_lab = ["aem_cis_j_st_trunc", "aem_csmc_j_st_trunc"]
        
    file_dir = os.path.join(base_dir, 'all_st_trunc')
    
    # For loading q
    is_lab = ["aem_is_j_st_trunc"]
    ...
    for i in range(args.reps):
        train_loader, test_loader = load_data(data_name, args)
        ...
        for j, (crit, lab) in enumerate(zip(crits, crit_lab)):
            ...
            ll[i, j], ll_sterr[i, j] = run_test(test_loader, file, save_dir, save_dir_q, min_val, max_val, args)
...
def run_test(test_loader, file, save_dir, save_dir_q, min_val, max_val, args):
    # Create energy net
