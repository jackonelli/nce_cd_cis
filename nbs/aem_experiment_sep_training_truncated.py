...
from src.noise_distr.aem_proposal_joint_z_no_context_truncated import AemJointProposalWOContextTruncated
...
def main(args):
    ...
    crit_dir = {
        'is': ([AemIsJointCrit], ["aem_is_j_st_trunc"]),
        'cis': ([AemCisJointCrit], ["aem_cis_j_st_trunc"]),
        'pers': ([AemCisJointPersCrit], ["aem_cis_pers_j_st_trunc"]),
        'adaptive': ([AemCisJointAdaCrit], ["aem_cis_adapt_j_st_trunc"]),
        'pers_adaptive': ([AemCisJointAdaPersCrit], ["aem_cis_pers_adapt_j_st_trunc"]),
        'smc': ([AemSmcCrit], ["aem_smc_j_st_trunc"]),
        'csmc': ([AemSmcCondCrit], ["aem_csmc_j_st_trunc"]),
        'csmc_pers': ([AemSmcCondPersCrit], ["aem_csmc_pers_j_st_trunc"]),
        'smc_adaptive': ([AemSmcAdaCrit], ["aem_smc_adapt_j_st_trunc"]),
        'csmc_adaptive': ([AemSmcCondAdaCrit], ["aem_csmc_adapt_j_st_trunc"]),
    }
    ...
    for i in range(args.reps):
        ...
        for j, (crit, lab) in enumerate(zip(crits, crit_lab)):
            ...
            min_val = torch.tensor(train_loader.loader.dataset.data.min(axis=0)) - 5
            max_val = torch.tensor(train_loader.loader.dataset.data.max(axis=0)) + 5
            #print(min_val > torch.tensor(validation_loader.dataset.data.min(axis=0)))
            #print(max_val < torch.tensor(validation_loader.dataset.data.max(axis=0)))
            #print(max_val[0])
            #print(torch.tensor(validation_loader.dataset.data.max(axis=0))[0])

            run_train(train_loader, validation_loader, crit, save_dir, save_dir_q, min_val, max_val, args)
            ll[i, j], ll_std[i, j] = run_test(test_loader, crit, save_dir, save_dir_q, min_val, max_val, args)
...
def run_train(train_loader, validation_loader, criterion, save_dir, save_dir_q, min_val, max_val, args):
    ...
    # Create proposal
    proposal = AemJointProposalWOContextTruncated(
    ...
    if args.alpha_warm_up_steps > 0:
        # Train q only (n_total_steps = alpha_warm_up_steps)
        print("Training proposal.")
        train_aem_model(crit, train_loader, validation_loader, save_dir_q, decaying_lr=True,
                        num_training_steps=args.alpha_warm_up_steps, num_warm_up_steps=args.alpha_warm_up_steps, hard_warmup=args.hard_alpha_warm_up,
                        lr=args.learning_rate, validation_freq=args.monitor_interval, device=device)
                    
    # Train p only (alpha_warm_up_steps = 0)
    if os.path.exists(os.path.join(save_dir_q, "proposal_final")):
        ...
        print("No proposal model found.")
    
    crit = criterion(model, proposal, args.n_proposal_samples_per_input, args.n_proposal_samples_per_input_validation)
    
    train_aem_model(crit, train_loader, validation_loader, save_dir, decaying_lr=True,
                    num_training_steps=args.n_total_steps, num_warm_up_steps=0, num_training_steps_q=0, hard_warmup=args.hard_alpha_warm_up,
                    lr=args.learning_rate, validation_freq=args.monitor_interval, device=device)



def run_test(test_loader, criterion, save_dir, save_dir_q, min_val, max_val, args):
    # Create energy net
