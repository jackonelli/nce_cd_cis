...
def run_test(test_loader, criterion, save_dir, save_dir_q, args):
   ...
   # Create energy net
    ...
    file = open("{}/eval_{}_set_smc_test.txt".format(save_dir, 'test'), "w")
    ...
    log_prob_est_all = torch.concat(tuple(log_prob_unnorm_est_all_ex)).cpu().numpy() - log_normalizer
    log_prob_proposal_all = torch.concat(tuple(log_prob_proposal_all_ex)).cpu().numpy()

    print(
        "Std of log normalizer with {} samples: {}".format(
            num_est, log_norm.cpu().numpy().std()
        ),
        file=file)
    print(log_norm.cpu().numpy().std()) 
    # Test log. normalizer
    rep = 100
    num_neg = [100, 500, 1000, 5000]#, 10000]#, 50000, 100000]#, 100000, 500000]#, 1000000, 2000000, 3000000]
    ...
    print(
        "Log normalizers, mean: {}".format(log_norm.mean(axis=-1)
        ),
        file=file)
    print(
        "Log normalizers, std: {}".format(log_norm.std(axis=-1)
        ),
        file=file)
                
    plt.errorbar(np.array(num_neg), log_norm.mean(axis=-1), yerr=log_norm.std(axis=-1))
    plt.savefig("log_normalizer_smc_st_" + args.dataset_name + "_" + args.criterion)
