...
def run_test(test_loader, criterion, save_dir, args):
    ...
    # Obs: nyligen ändrat till att skattningen är över medelvärde
    log_prob_est_all = torch.concat(tuple(log_prob_unnorm_est_all_ex)).cpu().numpy() - log_norm.mean().item() #log_normalizer
    ...
    # Test log. normalizer
    #rep = 100
    #num_neg = [100, 1000, 10000, 100000, 500000]#, 1000000, 2000000, 3000000]
...
#    log_norm = log_norm.cpu().numpy()
 ...
 #   plt.savefig("log_normalizer_smc_2_" + args.dataset_name + "_" + args.criterion)
