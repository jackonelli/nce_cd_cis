...
def main(args):
    ...
    for i in range(args.reps):
        ...
        for j, (crit, lab) in enumerate(zip(crits, crit_lab)):
            ...
            run_train(train_loader, validation_loader, crit, save_dir, save_dir_q, args)
            #ll[i, j], ll_std[i, j] = run_test(test_loader, crit, save_dir, save_dir_q, args)
...
def load_data(name, args):
    ...
    # training set
    train_dataset = data_uci.load_uci_dataset(name, split='train', num_dims=args.dims, scaling='normalize')
    ...
    # validation set
    val_dataset = data_uci.load_uci_dataset(name, split='val', frac=args.val_frac, num_dims=args.dims, scaling='normalize')
...
def run_train(train_loader, validation_loader, criterion, save_dir, save_dir_q, args):
    ...
    if os.path.exists(os.path.join(save_dir_q, "proposal_best")):
        print("Loading proposal.")
        proposal.load_state_dict(torch.load(os.path.join(save_dir_q, "proposal_best"), map_location=device)) # TODO: check so that this loads params also of made
    else:
