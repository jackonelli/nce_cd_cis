...
def run_test(test_loader, criterion, save_dir, save_dir_q, args):
   ...
   # Create energy net
    ...
    if dim == 2:
        ...
        H, xedges, yedges = np.histogram2d(batch_all[:, 0], batch_all[:, 1])
        X, Y = np.meshgrid(xedges, yedges)
        ax[0].pcolormesh(X, Y, H)
        
        ax[1].scatter(batch_all[:, 0], batch_all[:, 1], c=np.exp(log_prob_est_all_unnorm.sum(axis=-1) - log_prob_est_all_unnorm.sum(axis=-1).max()))
        ax[2].scatter(batch_all[:, 0], batch_all[:, 1], c=np.exp(log_proposal_est_all[:, 0]), label="Proposal")
        ...
        ax[1].set_xlabel("x_1")
        ax[1].set_ylabel("x_2")
        ax[1].set_title("Predicted (scaled)")
        ax[2].set_xlabel("x_1")
        ax[2].set_ylabel("x_2")
        ax[2].set_title("Proposal")

        plt.savefig(save_dir + "/dim_zero_one_hist_" + args.dataset_name + "_" + args.criterion)

    else:
        plt.hist(batch_all[:, 0], label="True", density=True)
        plt.plot(batch_all[:, 0], np.exp(log_prob_est_all_unnorm[:, 0]) / np.exp(log_prob_est_all_unnorm[:, 0]).max(), 'o', label="Predicted (scaled)")
