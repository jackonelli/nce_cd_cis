...
def main(args):
    ...
    crits = ['IS', 'CIS', 'CSMC']
    crit_lab = ["aem_is_j_st"]#, "aem_cis_j_st", "aem_csmc_j_st"]
        
    file_dir = os.path.join(base_dir, 'all_st')
    ...
    for i in range(args.reps):
        ...
        for j, (crit, lab) in enumerate(zip(crits, crit_lab)):
            ...
            if check_proposal(save_dir, save_dir_q, test_loader.dataset.dim, args):
                print("Proposal ok")
            else:
                print("Wrong proposal used?")

            ll[i, j], ll_sterr[i, j] = run_test(test_loader, file, save_dir, save_dir_q, args)
...
def check_proposal(save_dir, save_dir_q, dim, args):
    ...
    proposal.load_state_dict(torch.load(os.path.join(save_dir_q, "proposal_best"), map_location=device)) 
    
    
    # Actually used proposal
    
    # Create MADE
    made_2 = ResidualMADEJoint(
        input_dim=2*dim,
        n_residual_blocks=args.n_residual_blocks_made,
        hidden_dim=args.hidden_dim_made,
        output_dim_multiplier=3 * args.n_mixture_components,
    ...
    # Create proposal
    proposal_2 = AemJointProposalWOContext(
        autoregressive_net=made_2,
        num_components=args.n_mixture_components,
        mixture_component_min_scale=args.mixture_component_min_scale,
        apply_context_activation=args.apply_context_activation
    )
    
    proposal_2.load_state_dict(torch.load(os.path.join(save_dir, "proposal"), map_location=device)) 
    
    proposals_same = True
    for param_1, param_2 in zip(proposal.parameters(), proposal_2.parameters()):
        #print(torch.allclose(param_1, param_2))
        #print(torch.abs(param_1 - param_2).max())
        if not torch.allclose(param_1, param_2):
            proposals_same = False
    
    return proposals_same


def run_test(test_loader, file, save_dir, save_dir_q, args):
    # Create energy net
