...
def main(args):
    ...
    if args.n_mixture_components < 10:
        crit_lab = [cl + "_num_comp_" + str(args.n_mixture_components) for cl in crit_lab]
...
def run_test(test_loader, save_dir, save_dir_q, args):
    ...
    with torch.no_grad():
        ...
        y_s_all = torch.zeros((dim, batch_size, num_samples, dim))
        ancestor_inds_all = torch.zeros((dim, batch_size, num_samples)).long()
        log_p_tilde_y_s_all = torch.zeros((dim, batch_size, num_samples))
        log_q_y_s_all = torch.zeros((dim, batch_size, num_samples))
        log_w_tilde_y_s_all = torch.zeros((dim, batch_size, num_samples))
        ...
        ancestor_inds_all[0, :, :] = torch.arange(0, num_samples).reshape(1, -1).repeat(batch_size, 1)
        y_s_all[0, :, :, :] = y_s.clone()
        log_p_tilde_y_s_all[0, :, :] = log_p_tilde_y_s.reshape(-1, num_samples).clone()
        log_q_y_s_all[0, :, :] = log_q_y_s.reshape(-1, num_samples).clone()
        log_w_tilde_y_s_all[0, :, :] = log_w_tilde_y_s.clone()
        ...
        for i in range(1, dim):
            ...
            if resampling_inds.sum() > 0:
                ...
                with torch.no_grad():
                    ...
                    ancestor_inds_all[i, resampling_inds, :] = ancestor_inds.clone() 
                 
                    y_s[resampling_inds, :, :i] = torch.gather(
                        y_s[resampling_inds, :, :i], dim=1,
                        index=ancestor_inds[:, :, None].repeat(1, 1, i))     

            ancestor_inds_all[i, ~resampling_inds, :] = torch.arange(0, num_samples).reshape(1, -1).repeat(batch_size - resampling_inds.sum(), 1)
            ...
            y_s_all[i, :, :, :] = y_s.clone()
            log_p_tilde_y_s_all[i, :, :] = log_p_tilde_y_s.reshape(-1, num_samples).clone()
            log_q_y_s_all[i, :, :] = log_q_y_s.reshape(-1, num_samples).clone()
            log_w_tilde_y_s_all[i, :, :] = log_w_tilde_y_s.clone()
        ...
        for i in np.arange(dim - 1, -1, step=-1):
            y_values = torch.gather(y_s_all[i, 0, :, :], dim=0, index=select_ind[:, None].repeat(1, dim))
            y = torch.repeat_interleave(y_values, num_new, dim=0)
            w_y_s = torch.exp(log_w_tilde_y_s_all[i, :, :] - torch.logsumexp(log_w_tilde_y_s_all[i, :, :], dim=-1, keepdim=True))
            ...
            for j, axis in enumerate(ax.reshape(-1)):
                if j < plot_samp:
                    ...
                    axis.plot(y_values[j, i].cpu(), np.exp(log_p_tilde_y_s_all[i, 0, select_ind[j]].cpu()), 'o', c='green')
                    axis.plot(y_values[j, i].cpu(), np.exp(log_q_y_s_all[i, 0, select_ind[j]].cpu()), 'o', c='yellow')
                    axis.set_title("{}. ancestor = {}, w = {:.3f}, w unnorm = {:.3f}".format(select_ind[j], ancestor_inds_all[i, 0, select_ind[j]], w_y_s[0, select_ind[j]], log_w_tilde_y_s_all[i, 0, select_ind[j]]))
                else:
                    break
            ...
            if i > 0:
                select_ind = torch.tensor([ancestor_inds_all[i, 0, s] for s in select_ind])
