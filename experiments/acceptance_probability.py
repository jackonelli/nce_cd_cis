...
from src.utils.ring_model_exp_utils import (
    generate_true_params,
    initialise_params,
    get_cnce_noise_distr_par,
)
...
def main(args):
    ...
    data_dir = args.save_dir / "datasets"
    ...
    error_res = np.zeros(
        (
            len(configs),
            int(np.ceil(args.num_samples / BATCH_SIZE) * args.num_epochs) + 1,
            args.num_runs,
        )
    )
    acc_prob_res = np.zeros(
        (
            len(configs),
            2,
            int(np.ceil(args.num_samples / BATCH_SIZE) * args.num_epochs),
            args.num_runs,
        )
    )
    ...
    for rep in range(args.num_runs):
        ...
        training_data = RingModelDataset(
            ...
            precision=precision,
            data_path=data_dir
            / f"ring_data_size_{args.num_samples}_nn_{args.num_neg_samples}_rep_{rep}",
        )
        ...
        for i, config in enumerate(configs):
            ...
            if config["mcmc_steps"] is not None:
                criterion = config["criterion"](
                    ...
                    config["mcmc_steps"],
                    save_acc_prob=config["calc_acc_prob"],
                )
            else:
                criterion = config["criterion"](
                    p_m, p_n, args.num_neg_samples, save_acc_prob=config["calc_acc_prob"]
                )

            _, error_res[i, :, rep] = train_model(
                criterion,
                error_metric,
                train_loader,
            ...
            save_dir_pre = os.path.join(
                args.save_dir, config["name"] + "_num_neg_" + str(args.num_neg_samples)
            )
            if config["calc_acc_prob"]:
                acc_prob_res[i, 0, :, rep] = np.load(
                    Path(str(save_dir_pre) + "_" + "cd_cnce" + "_acc_prob.npy")
                ).mean(axis=0)
                acc_prob_res[i, 1, :, rep] = np.load(
                    Path(str(save_dir_pre) + "_" + "cd_mh" + "_acc_prob.npy")
                ).mean(axis=0)
...
def plot_acc_prob_res(configs, error_res, acc_prob_res, args):
    ...
    for i, config in enumerate(configs):
        ...
        if config["label"] in ["CNCE", "P-CNCE"]:
            other_col, other_lab = (
                ([124 / 255, 161 / 255, 204 / 255], "MH-CNCE")
                if config["label"] == "CNCE"
                else ([238 / 255, 186 / 255, 180 / 255], "P-MH-CNCE")
            )
            ...
            other_col, other_lab = (
                ([31 / 255, 68 / 255, 156 / 255], "CNCE")
                if config["label"] == "MH-CNCE"
                else ([240 / 255, 80 / 255, 57 / 255], "P-CNCE")
            )
        ...
        #     os.path.join(
        #         args.save_dir,
        #         "cnce_acc_prob_res_num_samples_"
...
def parse_args():
    parser = argparse.ArgumentParser("Ring model experiments with (pers.) (MH-)CNCE.")
    parser.add_argument("--num-epochs", default=50, type=int, help="Number of epochs to train for.")
    parser.add_argument("--num-runs", default=100, type=int, help="Number of runs to average over.")
    parser.add_argument("--num-samples", default=200, type=int, help="Number of training samples.")
    parser.add_argument(
        "--num-neg-samples",
        default=5,
        type=int,
        help="Number of negative examples used in criteria.",
    )
    parser.add_argument(
        "--base-lr",
        default=0.01,
